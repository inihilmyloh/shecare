...
if (!isset($input['id_penyakit']) || empty($input['id_penyakit'])) {
    ...
    echo json_encode(['status' => false, 'message' => 'id_penyakit diperlukan']);
    exit;
}

if (!isset($input['nama']) || empty($input['nama'])) {
    http_response_code(400);
    echo json_encode(['status' => false, 'message' => 'nama diperlukan']);
    exit;
}

if (!isset($input['label']) || empty($input['label'])) {
    http_response_code(400);
    echo json_encode(['status' => false, 'message' => 'label diperlukan']);
    exit;
}

if (!isset($input['tipe_input']) || empty($input['tipe_input'])) {
    http_response_code(400);
    echo json_encode(['status' => false, 'message' => 'tipe_input diperlukan']);
    exit;
}
